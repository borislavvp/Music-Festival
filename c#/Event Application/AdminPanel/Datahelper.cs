...
namespace AdminPanel
{
    class Datahelper
    {
        ...
        public List<string> GetVisitorsNo()
        {
            List<string> visitorsNo = new List<string>();
            try
            {

                string sql = $@"SELECT User_userID FROM eventacc ;";
                MySqlCommand command = new MySqlCommand(sql, connection);
                connection.Open();
                MySqlDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
                    visitorsNo.Add(reader[0].ToString());
                }
                return visitorsNo;
            }
            catch
        ...
        public List<string> GetCampingsNo()
        {
            List<string> campings = new List<string>() ;
            try
            {

                string sql = $@"SELECT SpotID FROM camping ;";
                MySqlCommand command = new MySqlCommand(sql, connection);
                connection.Open();
                MySqlDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
                    campings.Add(reader[0].ToString());
                }
                return campings;
            }
            catch
        ...
        public List<string> GetProductsForReStock()
        {
            List<string> products = new List<string>();
            try
            {
                string sql = $@"SELECT ProductName 
                                FROM rentshop_has_items WHERE StockQuantity <= 5;";
                MySqlCommand command = new MySqlCommand(sql, connection);
                connection.Open();
                MySqlDataReader reader = command.ExecuteReader();
                while (reader.Read())
                ...
                sql = $@"SELECT ProductName 
                         FROM shop_has_items WHERE StockQuantity <= 5;";
                command = new MySqlCommand(sql, connection);
                reader = command.ExecuteReader();
                while (reader.Read())
                {
        ...
        public string UpdateStockQuantity(string ProductName)
        {
            try
            {
                bool isFromRentShop = false;

                string sql = $@"SELECT * 
                                FROM rentshop_has_items WHERE ProductName = '{ProductName}';";
                MySqlCommand command = new MySqlCommand(sql, connection);
                connection.Open();
                MySqlDataReader reader = command.ExecuteReader();
                if (reader.HasRows)
                ...
                reader.Close();
                if (isFromRentShop == true)
                {
                    sql = $@"UPDATE rentshop_has_items SET StockQuantity = '30' WHERE ProductName = '{ProductName}' ; ";
                    command = new MySqlCommand(sql, connection);
                    int number = 0;
                ...
                {
                    sql = $@"UPDATE shop_has_items SET StockQuantity = '30' WHERE ProductName = '{ProductName}' ; ";
                    command = new MySqlCommand(sql, connection);
                    int number = 0;
        ...
        public int GetAmountEarnedFromTombola()
        {
            int amountEarned = 0;
            try
            {

                string sql = $@"SELECT EarnedMoney FROM tombolawinner ;";
                MySqlCommand command = new MySqlCommand(sql, connection);
                connection.Open();
                MySqlDataReader reader = command.ExecuteReader();
                if (!reader.HasRows)
                ...
                reader.Read();
                amountEarned = int.Parse(reader[0].ToString());
